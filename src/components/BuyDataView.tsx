...
const BuyDataView = ({
  ...
  return <div className="min-h-screen bg-gray-50 overflow-y-auto">
      ...
      <div className="-mt-6 my-0 py-[31px] px-4 sm:px-[24px]">
      ...
      <div className="px-4 pb-8">
        <div className="bg-white rounded-2xl shadow-sm overflow-hidden p-4 sm:p-6 space-y-6">
          <div className="space-y-2">
            <label className="text-[#070058] text-lg font-semibold block">Enter Phone Number</label>
            <div className="relative">
              <Input type="tel" placeholder="07xxxxxxxxx" value={phoneNumber} onChange={e => handlePhoneNumberChange(e.target.value)} className="h-12 bg-gray-50 rounded-xl text-base placeholder:text-gray-400 border-0 pr-24" disabled />
              {recentNumbers.length > 0 && <Select value={phoneNumber} onValueChange={setPhoneNumber} disabled>
                  ...
                  <SelectContent>
                    {recentNumbers.map(number => <SelectItem key={number} value={number}>
                        {number}
                      </SelectItem>)}
                  </SelectContent>
                </Select>}
          ...
          <Tabs defaultValue="hot" className="w-full" onValueChange={setSelectedTab}>
            <div className="overflow-x-auto pb-2">
              <TabsList className="w-full min-w-max grid grid-cols-4 gap-2 bg-transparent h-auto p-0">
                <TabsTrigger value="hot" disabled className="data-[state=active]:bg-[#070058] data-[state=active]:text-white border rounded-lg py-2 px-2 sm:px-4 whitespace-nowrap">
                  Hot Deals
                </TabsTrigger>
                <TabsTrigger value="daily" disabled className="data-[state=active]:bg-[#070058] data-[state=active]:text-white border rounded-lg py-2 px-2 sm:px-4 whitespace-nowrap">
                  Daily
                </TabsTrigger>
                <TabsTrigger value="weekly" disabled className="data-[state=active]:bg-[#070058] data-[state=active]:text-white border rounded-lg py-2 px-2 sm:px-4 whitespace-nowrap">
                  Weekly
                </TabsTrigger>
                <TabsTrigger value="monthly" disabled className="data-[state=active]:bg-[#070058] data-[state=active]:text-white border rounded-lg py-2 px-2 sm:px-4 whitespace-nowrap">
                  Monthly
                </TabsTrigger>
              </TabsList>
            </div>

            {Object.entries(dataPlansByCategory).map(([category, plans]) => <TabsContent key={category} value={category} className="mt-6">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {plans.map(plan => <button key={plan.id} onClick={() => handlePlanSelection(plan)} disabled className="bg-gray-50 rounded-xl p-4 text-center space-y-2 hover:bg-gray-100 transition-colors opacity-70 cursor-not-allowed w-full mx-0 px-0">
                      <div className="text-xs text-gray-500 capitalize">{plan.category}</div>
                      <div className="space-y-1">
                        {plan.features.map((feature, idx) => <div key={idx} className="text-[#070058] font-semibold text-sm sm:text-base">
                            {feature}
                          </div>)}
                      </div>
