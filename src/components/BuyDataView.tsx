            ...
            <div className="overflow-x-auto pb-2 scrollbar-hide">
              <TabsList className="w-full min-w-max grid grid-cols-4 gap-2 bg-transparent h-auto p-0">
                <TabsTrigger 
                  value="hot" 
                  disabled 
                  className="data-[state=active]:bg-[#070058] data-[state=active]:text-white border rounded-lg py-2 px-2 sm:px-4 whitespace-nowrap"
                >
                  Hot Deals
                </TabsTrigger>
                <TabsTrigger 
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {plans.map(plan => (
                    <button 
                      ...
                      className="bg-gray-50 rounded-xl p-4 text-center space-y-2 hover:bg-gray-100 transition-colors opacity-70 cursor-not-allowed w-full" 
                      disabled
                    >
                      <div className="text-xs text-gray-500 capitalize">{plan.category}</div>
                      <div className="space-y-1">
                        {plan.features.map((feature, idx) => (
                          <div key={idx} className="text-[#070058] font-semibold text-sm sm:text-base">
                            {feature}
                          </div>
                        ))}
                      </div>
